...
class UjianController extends Controller
{
    ...
    public function mulaiUjian($id)
    {
        ...
        if(Ujian::where('id_siswa', $siswa->id)->where('id_mapel', $id)->exists()) {
            ...
            return view('pages.ujian.mulai', compact('mapel', 'soal', 'jadwal'));
        }
    ...
    public function kirimJawaban(Request $request)
    {
        ...
        $jumlah_soal = Jadwal::where('id_jadwal', $request->id_jadwal)->get();
         
        $hasil = Hasil::create([
            'id_siswa' => $siswa->id,
            'id_mapel' => (int)$request->id_mapel,
            'id_jadwal' => $request->id_jadwal,
            'nilai_final' => Ujian::where('id_siswa', $siswa->id)->where('id_mapel', $request->id_mapel)->where('betul', 1)->count('betul') * 100 / $jumlah_soal,
        ]);
// jumlah benar bagi jumlah seluruh soal kali 100 da cari berapa soal nya 
